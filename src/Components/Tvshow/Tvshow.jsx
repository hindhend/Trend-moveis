...
export default function Tvshow() {
 ...
 let nums= new Array(10).fill(1).map((ele,index)=>index+1);
  ...
  async function getTrending(page){
    ...
    let {data}= await axios.get( `https://api.themoviedb.org/3/discover/tv?api_key=99d7f41a074df442999f3e145755a6b6&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=true&page=${page}`)
  ...
  useEffect(()=>{
    ...
    {tv?    <div className="row py-3">
     {tv?.map((item,index)=>{
      ...
      <img src={'https://image.tmdb.org/t/p/w500'+item.poster_path} className='w-100' alt={item.name}/>
